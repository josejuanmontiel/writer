...
import { CreateCompendiumModule } from '../../wailsjs/go/main/App';
...
export default function NewModuleModal({
  ...
  const handleTitleChange = (e) => {
    ...
    setTitle(val);
    if (!slug || slug.startsWith('modulo-')) {
      const generatedSlug = val
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)+/g, '');
      setSlug(generatedSlug || 'modulo-nuevo');
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!slug.trim()) {
      setError('Debes especificar un identificador para la carpeta del módulo.');
      return;
    }
    if (!title.trim()) {
    ...
    try {
      await CreateCompendiumModule(
        slug.trim(),
        title.trim(),
        description.trim()
      );
      if (onModuleCreated) {
        onModuleCreated(slug.trim());
      }
      onClose();
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">
                Nuevo Módulo Temático
              </h3>
              <p className="text-xs text-slate-400">
                Añadir una unidad o bloque de contenido al compendio
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej. Módulo 2: Sistema Hidráulico y Motores"
          ...
          <div>
            <label className="block text-slate-300 font-medium mb-1.5">
              Identificador de Carpeta en content/ <span className="text-rose-400">*</span>
            </label>
            <input
              type="text"
              required
              ...
              placeholder="modulo-02-hidraulica"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Breve introducción de lo que cubrirá este módulo..."
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
            ...
            >
              ...
              <span>{isSubmitting ? 'Creando...' : 'Crear Módulo'}</span>
            </button>
          </div>
        </form>
