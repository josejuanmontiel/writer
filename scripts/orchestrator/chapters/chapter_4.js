  ...
  scenes: [
    {
      ...
      action: async (page, durationMs, mcp) => {
        // Enfocar la barra lateral izquierda y la sección de ideas flotantes
        await page.mouse.move(120, 300, { steps: 15 });
        await page.waitForTimeout(500);
        await page.mouse.move(120, 480, { steps: 15 });
      }
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        const addFloatingBtn = await page.$('button[title*="flotante"], button[title*="Flotante"], button[title*="Borrador"]');
        if (addFloatingBtn) {
          await addFloatingBtn.click({ force: true });
          await page.waitForTimeout(600);
        }
        await page.mouse.move(150, 520, { steps: 15 });
      }
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        ...
        await page.mouse.move(140, 530, { steps: 15 });
      }
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        await page.mouse.move(150, 500, { steps: 15 });
        await page.waitForTimeout(600);
