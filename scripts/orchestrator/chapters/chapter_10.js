  ...
  scenes: [
    {
      ...
      action: async (page, durationMs, mcp) => {
        ...
        const wizardBtn = await page.$('button[title*="Wizard"], button[title*="Asistente"], button:has-text("Wizard")');
        if (wizardBtn) {
    ...
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        const studioBtn = await page.$('button[title*="Studio"], button[title*="Prompt"]');
        if (studioBtn) {
    ...
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        const gitBtn = await page.$('button[title*="Git"], button[title*="Sync"], button[title*="Sincronizar"]');
        if (gitBtn) {
          await gitBtn.click();
          await page.waitForTimeout(1000);
          await page.keyboard.press('Escape');
