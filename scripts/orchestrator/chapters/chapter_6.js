...
export const chapter6Data = {
  ...
  scenes: [
    ...
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        // Clic en icono brújula de la idea flotante
        const compassBtn = await page.$('button[title*="Reubicación"], button[title*="Reubicar"], button:has(.lucide-compass)');
        if (compassBtn) {
          await compassBtn.click({ force: true });
    ...
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        await page.mouse.move(500, 320, { steps: 15 });
    ...
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        await page.mouse.move(500, 420, { steps: 15 });
        await page.waitForTimeout(600);
      }
    },
    {
      ...
      action: async (page, durationMs, mcp) => {
        await page.mouse.move(650, 40, { steps: 15 });
      }
    }
  ]
};
